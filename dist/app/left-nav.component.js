"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var LeftNavComponent = (function () {
    function LeftNavComponent() {
        this.changeBtn = new core_1.EventEmitter();
        this.iteamArray = [
            { 'name': 'item1' }
        ];
        this.iteamCount = 0;
        this.addIteam();
    }
    LeftNavComponent.prototype.itemSelected = function (showButton, event) {
        this.changeBtn.emit(showButton);
        for (var i = 0; i < event.target.parentNode.children.length; i++) {
            event.target.parentNode.children[i].className = "collection-item";
        }
        event.target.setAttribute("class", "pan_collection_active");
    };
    LeftNavComponent.prototype.addIteam = function () {
        for (var i = this.iteamCount; i < this.iteamCount + 20; i++) {
            this.iteamArray.push({ 'name': "item " + i });
        }
        this.iteamCount = this.iteamCount + 20;
    };
    LeftNavComponent.prototype.onScroll = function () {
        this.addIteam();
    };
    __decorate([
        core_1.Output(), 
        __metadata('design:type', Object)
    ], LeftNavComponent.prototype, "changeBtn", void 0);
    LeftNavComponent = __decorate([
        core_1.Component({
            selector: 'my-left-nav',
            templateUrl: './dist/view/left-nav.component.html'
        }), 
        __metadata('design:paramtypes', [])
    ], LeftNavComponent);
    return LeftNavComponent;
}());
exports.LeftNavComponent = LeftNavComponent;
//# sourceMappingURL=left-nav.component.js.map